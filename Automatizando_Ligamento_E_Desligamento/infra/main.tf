...
resource "aws_autoscaling_schedule" "liga" {
	...
	min_size               = var.minimoLigado
	max_size               = var.maximoLigado
	desired_capacity       = var.quandidadeLigadoDesejada
	start_time             = timeadd(timestamp(), "10m")
	recurrence	           = var.recurrenceLigado
...
resource "aws_autoscaling_schedule" "desliga" {
	...
	recurrence	           = var.recurrenceDesligado
	autoscaling_group_name = aws_autoscaling_group.grupo.name
}
